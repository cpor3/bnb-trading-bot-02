...
SL = 1 - 0.01 # 1% por abajo del precio
...
#					mostrar_dashboard()
					...
					SL_price = precio_actual * SL
					SL_stopPrice = SL_price

					print('Colocando orden de venta SL:', qty, PAR, 'a', SL_price, ', Stop Loss en', SL_stopPrice)
					orden_venta = client.order_limit_sell(
						symbol = PAR, 
						quantity = qty, 
						price = SL_price,
					...
					while (orden_venta['status'] != 'FILLED') and on_time:
						...
						client.order_limit_sell_simulacion(
						...
						nuevo_precio = precio(PAR)
						nuevo_SL = nuevo_precio * SL
